...
\begin{figure*}[!b]
    \centering
    \begin{subfigure}[t]{0.85\linewidth} % contains the two plots in a single figure
        \includegraphics[width=\linewidth]{Figures/orbits}
        \phantomcaption
        \label{fig:a_evo_ecc}
    \end{subfigure}
    \begin{subfigure}[t]{0\linewidth} % the hidden unwanted image
...
To summarise the above discussion, if we assume the thick ocean scenario from \citet{sohl2014structural} and \citet{baland2014titan}, then Figure \ref{fig:a_evo} indicates that eccentricity and obliquity induced gravity-wave resonances will not significantly reduce or reverse the rate of semimajor axis change for Titan. For optimal bottom drag coefficients, however, the depth independent Rossby-wave resonance is capable of reducing the Saturn induced outward migration rate by up to \SI{40}{\percent}.
