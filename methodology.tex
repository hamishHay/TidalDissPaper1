...
We employ a fixed latitude-longitude grid for our numerical simulations. The gird is constructed in a staggered manner, meaning that velocity nodes are placed to the east and south of their parent displacement nodes. This is illustrated in Figure \ref{fig:grid}. A staggered approach is reasonably common in CFD problems, as it avoids oscillations building up in the solution by calculating derivatives \textit{between} grid points, rather than over them. This is discussed further in section \ref{subsec:fd_expan}. 
