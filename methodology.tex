...
After $u$, $v$, and $\eta$ are solved for at the new time step, $t+1$, the energy calculations begin. For Rayleigh friction dissipation, we calculate and store,
\begin{equation}
...
The summed terms in Equation \ref{eq:E_alpha} give the total dissipated energy in the system at the current time step, while the factor in front averages this quantity over the satellites surface area giving a quantity in watts per square metre. For bottom friction dissipation the equivalent expression is,
\begin{equation}
